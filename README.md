...
1. Китайские дешевые камеры при использовании кодека h265, подмешивают аудио сигнал в видеопоток. 
<br>
Поэтому ffmpeg не видит аудиодорожку. 
Для решения проблемы нужно:
- Использовать либо специфичную сборку ffmpeg, которая обрабатывает SEI сообщения,
- или добавлять в контейнер конвертер [ipcam26Xconvert](https://github.com/francescovannini/ipcam26Xconvert),
- или аналогичный конвертер (поддерживаемый передачу потока файлов через ОЗУ).
<br>
2. Создать .env для токена от бота и для хранения внешнего ip адреса.
